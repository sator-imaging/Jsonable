        ...
        public static string EscapeStringIfRequired(string value)
        {
...
#if __supported
            // 128 chars threshold. stackalloc 256 bytes (128 chars) to accommodate potential doubling of length during escaping.
            if (value.Length <= 128)
            {
            ...
            static string ShortSlowPath(string value, int firstIndex)
            {
                ...
                while (true)
                {
                    ...
                    if (written + len + 2 > buffer.Length) goto FALLBACK;

                    span.Slice(lastIndex, len).CopyTo(buffer.Slice(written));
                    written += len;
                    ...
                    if (lastIndex >= span.Length) break;
                    currentIndex = span.Slice(lastIndex).IndexOfAny(EscapeTargets);
                ...
                {
                    int len = span.Length - lastIndex;
                    if (written + len > buffer.Length) goto FALLBACK;
...
#if __supported
            static string ShortSlowPath(ReadOnlySpan<byte> utf8, int currentIndex)
            {
                ...
                {
                    int len = currentIndex - lastIndex;
                    utf8.Slice(lastIndex, len).CopyTo(buffer.Slice(written));
                    written += len;
                    ...
                    if (currentIndex + 1 < utf8.Length)
                    {
                        ...
                        switch (next)
                        {
                            ...
                            case (byte)'t': buffer[written++] = (byte)'\t'; lastIndex = currentIndex + 2; break;
                    ...
                    if (lastIndex >= utf8.Length) break;
                    currentIndex = utf8.Slice(lastIndex).IndexOf((byte)'\\');
                    if (currentIndex < 0) break;
                    currentIndex += lastIndex;
                }
