    ...
    internal static class JSONABLE
    {
        ...
        public static string UnescapeStringIfRequired(ReadOnlySpan<byte> utf8)  // for .NET Standard 2.0; Encoding doesn't have span overload
        {
            ...
            static string ShortSlowPath(ReadOnlySpan<byte> utf8)
            {
                ...
                {
                    int len = currentIndex - lastIndex;
                    if (written + len + 1 > buffer.Length) goto FALLBACK;
            ...
            FALLBACK:
                return SlowPath(utf8, value);
            }
#endif

            static string SlowPath(ReadOnlySpan<byte> utf8, string? decoded = null)
            {
                var value = decoded ?? Encoder.GetString(utf8);
